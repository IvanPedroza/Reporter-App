...
let annealStart (inputParams : string list) (annealsForm : string) (reporter : ExcelWorksheet) (myTools : ExcelWorksheet) =
    ...
    let reagentsInput = "anneals" //Console.ReadLine ()

    let heatBlockInput = "heatblocks" //Console.ReadLine ()
    ...
    for param in inputParams do
        ...
        let tests = formPackage.GetAsByteArray()

        File.WriteAllBytes(path, tests)
